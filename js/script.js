...
doc.querySelector('.button__next').addEventListener('click', () => {
    offset += 1024
    if (offset > 2048) {
        offset = 0
    }
...
doc.querySelector('.button__prev').addEventListener('click', () => {
    offset -= 1024
    if (offset < 0) {
        offset = 2048
    }
...
const images = doc.querySelectorAll('.wrap__slider .slider img')
const slide = doc.querySelector('.slider')
let count = 0
let width;

function init() {
    console.log('resize');
    width = doc.querySelector('.wrap__slider').offsetWidth
...
function rollSlider() {
    slide.style.left = -count * width + 'px'
}
